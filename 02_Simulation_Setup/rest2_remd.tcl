...
if {[info exists restart_root]} { #restart
  ...
  #Even replicas have location a upwards, location b downwards.
  #Odd replicas the other way round. (Important for swapping event)
  if { $r % 2 == 0 && $r+1 < $nr } {
...
proc replica_lambda { i } {
  #Calculates the potential energy scaling factor for a given replica
...
if {$i_run} { #restart the replica run
...
puts stderr "EXCHANGE_ACCEPT index swap RUN irun"
puts stderr "BENCHMARK_EXCHANGE_RATIO index swap accepts attempts ratio"

while {$i_run < $num_runs} {
# On even attempts swap with partner on location loc.a
  if { $i_run % 2 == 0 } { 
...
#  flush $history_file
  ...
  #output benchmark exchange ratios regularly
  if { ( $i_run % 20 == 0 && $i_run < 100 ) || $i_run % 100 == 0 } {
    ...
    puts stderr "BENCHMARK_EXCHANGE_RATIO $replica(index) $replica(index.$swap) $accepts $attempts $ratio"
    }
  }
}
...
if $attempts {
  ...
  set accepts $replica(exchanges_accepted)
  set ratio [expr 1.0*$accepts/$attempts]
  puts stderr "EXCHANGE_RATIO index swap accepts attempts ratio"
